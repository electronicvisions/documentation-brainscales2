...
The coordinate :cpp:expr:`halco::hicann::v2::HICANNOnWafer` refers to a single chip on a BrainScaleS-1 wafer.
...
The coordinate connected to a single neuron circuit is the :cpp:expr:`halco::hicann_dls::vx::v2::AtomicNeuronOnDLS`.
...
:cpp:expr:`halco::hicann::v2::NeuronOnHICANN` represents a neuron circuit on a single neuromorphic chip.
In order to refer to a specific neuron circuit on the wafer the coordinate :cpp:expr:`halco::hicann::v2::NeuronOnWafer` is used which is a combination of the coordinates :cpp:expr:`halco::hicann::v2::NeuronOnHICANN` and :cpp:expr:`halco::hicann::v2::HICANNOnWafer`.

BrainScaleS-2 example
^^^^^^^^^^^^^^^^^^^^^

The BrainScaleS-2 chip is composed of an upper and lower half (called hemispheres).
On each hemisphere are two analog parameter storages (CapMemBlock) referenced by the coordinate :cpp:expr:`halco::hicann_dls::vx::v2::CapMemBlockOnHemisphere`.
To identify an analog parameter storage on the chip this coordinate is combined with :cpp:expr:`halco::hicann_dls::vx::v2::HemisphereOnDLS` to form :cpp:expr:`halco::hicann_dls::vx::v2::CapMemBlockOnDLS`.
