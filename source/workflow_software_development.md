...
* To set up a link to a directory on your remote inside the Windows Explorer, follow this [setup guide](https://github.com/winfsp/sshfs-win?tab=readme-ov-file#basic-usage). Alternatively you can also do it in the shell by following the guide right below. In order to map to your home directory on hel, you need to specify `\\sshfs\USERNAME@brainscales-r.kip.uni-heidelberg.de!11022` for the SSHFS path during the setup procedure. As described in the setup guide, it should be possible to amend this SSHFS path by the path of the target directory on hel in order to map to somewhere else than your home directory, but this fails (state 9th July 2025 using SSHFS-Win 2021 with WinFsp 2025).
